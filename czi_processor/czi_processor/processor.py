...
class CZIOutputFile(object):
    ...
    def load_and_save_assets(self, czi_file_path, i_xyzct=(-1, -1, -1, -1, -1), n_xyzct=(-1, -1, -1, -1, -1),
                             asset_format='dzi'):
        """Load image and generate view assets"""
        # Set file path
        self.file_path = czi_file_path
    ...
    def save_ometiff(self):
        ...
        # Write image
        try:
            assert self.ometiff_path.endswith('.ome.tiff')
        except AssertionError:
            raise TypeError("Output view path should be set to OME-TIFF")
            ...
            for z in range(self.SizeZ):
                for t in range(self.SizeT):
                    if ''.join(self.DimensionOrder) == 'XYCZT' and self.isRGB:
                        ...
                        # Convert type of image in order to save in deep-zoom
                        self.img_data = self._convert_image_data_type(self.img_data)
                        bf.formatwriter.write_image(self.ometiff_path, self.img_data, data_type,
                                                    z=z, t=t,
                                                    size_z=self.SizeZ, size_t=self.SizeT)
                    else:
...
class CZIProcessor(BaseMicroscopyImageProcessor):
    required_inputs = ['file']

    def __init__(self, *args, **kwargs):
        super(CZIProcessor, self).__init__(*args, **kwargs)
    ...
    def load_and_save(self, i_xyzct=(-1, -1, -1, -1, -1), n_xyzct=(-1, -1, -1, -1, -1)):
        ...
        if os.path.isfile(self.file):
            output_file = CZIOutputFile(optimize=self.optimize, tile_size=self.tile_size,
                                        tile_overlap=self.tile_overlap, tile_format=self.tile_format,
                                        image_quality=self.image_quality, resize_filter=self.resize_filter)
            output_file.load_and_save_assets(self.file, i_xyzct=i_xyzct, n_xyzct=n_xyzct)
            output_file.ometiff_path = 'output.ome.tiff'
            if i_xyzct == (-1, -1, -1, -1, -1) or i_xyzct == (0, 0, 0, 0, 0):
                output_file.save_ometiff()
            ...
            if (i_x, i_y, i_z, i_c, i_t) == (-1, -1, -1, -1, -1) or (i_x, i_y, i_z, i_c, i_t) == (0, 0, 0, 0, 0):
                with bf.ImageReader(path=self.outputs[0].file_path) as rdr:
                    if ''.join(self.outputs[0].DimensionOrder) == 'XYCZT' and self.outputs[0].isRGB:
                        img_data = np.zeros((self.outputs[0].SizeX, self.outputs[0].SizeY, 3))
                        ...
                        img_data = np.zeros((self.outputs[0].SizeX, self.outputs[0].SizeY, self.outputs[0].SizeC))
                        for z in range(self.outputs[0].SizeZ):
                            for t in range(self.outputs[0].SizeT):
                ...
                mip_img_data = np.zeros((self.outputs[0].SizeX, self.outputs[0].SizeY, 3))
